...
function compteur_chacrateres_header_prive($flux){
    ...
    $flux .= '<link id="cssprivee" href="'.find_in_path('css/compteur_chacrateres.css').'" type="text/css" rel="stylesheet">';
    return $flux;
}

function compteur_chacrateres_recuperer_fond($flux){
    //Insertion des onglets de langue
    $texte=$flux['data']['texte'];
    if ($flux['args']['fond'] == 'formulaires/editer_article'){
        $descriptif='<div class="compteur"><span class="compteur_descriptif"></span>'._T('compteur_caractres:caracteres').'</div>';
        $textarea='<div class="compteur"><span class="compteur_text_area"></span>'._T('compteur_caractres:caracteres').'</div>';        
